...
static inline uint64_t combine_keycode_with_subkey(uint64_t keycode, uint64_t subkey) {
    ...
    uint64_t lo = ((~subkey64) + 2ULL) & 0xFFFFULL;
...
static nanobind::bytes hca_decryptor_main(nanobind::bytes data, uint64_t mainkey, nanobind::object py_subkey) {
    const char *cptr = data.c_str();
    size_t len = data.size();
    std::vector<uint8_t> in_bytes(reinterpret_cast<const uint8_t *>(cptr), reinterpret_cast<const uint8_t *>(cptr) + len);

    HCAHeader H = parse_hca_header(in_bytes);

    uint64_t effective_key = mainkey & ((1ULL << 56) - 1);
    if (!py_subkey.is_none()) {
        uint64_t subkey = nanobind::cast<uint64_t>(py_subkey);
    ...
    const size_t new_hdr_sz = (out.size() >= 8) ? static_cast<size_t>((out[6] << 8) | out[7]) : 0;
...
void init_hca_decryptor(nanobind::module_ &m) {
    m.def("hca_decryptor_main", &hca_decryptor_main, nanobind::arg("data"), nanobind::arg("mainkey"), nanobind::arg("subkey") = nanobind::none(),
          R"pbdoc(
Decrypt HCA audio data.

Args:
    data (bytes): Encrypted HCA audio data.
    mainkey (int): Main decryption key.
    subkey (Optional[int]): Optional subkey for decryption.

Returns:
    bytes: Decrypted audio data.
)pbdoc");
}
