...
static std::vector<uint8_t> decrypt_hca_to_plain_bytes(const std::vector<uint8_t> &in_bytes, uint64_t mainkey, nanobind::object py_subkey)
{
...
void init_hca_decryptor(nanobind::module_ &m)
{
    ...
    // Python API: decrypt(data: bytes, mainkey: int, subkey: Optional[int]) -> bytes
    m.def(
        "decrypt",
        [](nanobind::bytes data, uint64_t mainkey, nanobind::object subkey) -> nanobind::bytes
        {
            ...
            return nanobind::bytes(out.data(), out.size());
        },
        nanobind::arg("data"),
        nanobind::arg("mainkey"),
        nanobind::arg("subkey") = nanobind::none(),
        R"pbdoc(
