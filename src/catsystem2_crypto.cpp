...
public:
    explicit MT(uint32_t seed) : mti(N)
    {
        for (int i = 0; i < N; ++i)
        {
            mt[i] = seed & 0xFFFF0000;
            seed = _32(69069ULL * seed + 1);
            mt[i] |= seed >> 16;
            seed = _32(69069ULL * seed + 1);
        }
    }
    ...
    uint32_t genrand()
    {
        if (mti >= N)
        {
            ...
            {
                uint32_t y = (mt[kk] & 0x80000000) | (mt[(kk + 1) % N] & 0x7FFFFFFF);
...
private:
    ...
    std::pair<uint32_t, uint32_t> Encipher(uint32_t xl, uint32_t xr)
    {
        for (int i = 0; i < N; ++i)
        {
            xl ^= P[i];
    ...
    std::pair<uint32_t, uint32_t> Decipher(uint32_t xl, uint32_t xr)
    {
        for (int i = N + 1; i >= 2; --i)
        {
            xl ^= P[i];
...
public:
    explicit Blowfish(nanobind::bytes key_bytes)
    {
        ...
        S[0] = {{0xD1310BA6, 0x98DFB5AC, 0x2FFD72DB, 0xD01ADFB7, 0xB8E1AFED, 0x6A267E96, 0xBA7C9045, 0xF12C7F99,
        ...
        S[1] = {{0x4B7A70E9, 0xB5B32944, 0xDB75092E, 0xC4192623, 0xAD6EA6B0, 0x49A7DF7D, 0x9CEE60B8, 0x8FEDB266,
        ...
        S[2] = {{0xE93D5A68, 0x948140F7, 0xF64C261C, 0x94692934, 0x411520F7, 0x7602D4F7, 0xBCF46B2E, 0xD4A20068,
        ...
        S[3] = {{0x3A39CE37, 0xD3FAF5CF, 0xABC27737, 0x5AC52D1B, 0x5CB0679E, 0x4FA33742, 0xD3822740, 0x99BC9BBE,
        ...
        for (size_t i = 0; i < 18; ++i)
        {
            ...
            for (int j = 0; j < 4; ++j)
            {
                val = (val << 8) | expandedKey[i * 4 + j];
            }
            keyData.push_back(val);
        }

        // XOR P array with key data
        for (int i = 0; i < 18; ++i)
        {
            P[i] ^= keyData[i];
        }
    ...
    nanobind::bytes decrypt(nanobind::bytes ciphertext_bytes)
    {
        ...
        for (size_t i = 0; i < ciphertext.size(); i += 8)
        {
            ...
            plaintext.push_back((xl >> 24) & 0xFF);
            plaintext.push_back(xr & 0xFF);
            plaintext.push_back((xr >> 8) & 0xFF);
            plaintext.push_back((xr >> 16) & 0xFF);
            plaintext.push_back((xr >> 24) & 0xFF);
        }

        return nanobind::bytes(plaintext.data(), plaintext.size());
    }
};
